// src/components/app/ExpensesView.tsx
...
export function ExpensesView({ transactions, members, month, currentUserId, onAdd, onDelete, onEdit }: Props) {
    const { t } = useI18n()
    ...
    const total = useMemo(() => monthExpenses.reduce((s, tx) => s + tx.amount, 0), [monthExpenses])
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        await onAdd({
            type: 'expense', amount: amt, description: desc.trim(),
    ...
    return (
        <div>
            <div className="fcard">
                <div className="fttl">{t.newExpenseTitle}</div>
                <form onSubmit={handleSubmit}>
                    <div className="fg fg2">
                        <div className="fl">
                            <label>{t.descriptionLabel}</label>
                            <input className="inp" value={desc} onChange={(e) => setDesc(e.target.value)} placeholder={t.expensePlaceholder} required />
                        </div>
                        <div className="fl">
                    ...
                    <button type="submit" className="sbtn">{t.addExpenseBtn}</button>
                </form>
            </div>

            <div className="sec-hd">
                <span className="sec-ttl">{t.expensesThisMonth}</span>
                {total > 0 && <span className="badge neg"><Money amount={total} sign="−" /></span>}
            </div>
            <div className="card">
                {monthExpenses.length === 0 ? (
                    <div className="empty"><p>{t.noExpenses}</p></div>
                ) : (
