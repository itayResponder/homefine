...
- `CategorySelect` (`src/components/ui/CategorySelect.tsx` + `.css`) — **the standard category picker for all transaction forms**; bottom sheet on mobile, centered modal on desktop; has search, category grid, add + edit mini-form; trigger styled identically to `CustomSelect` (uses `--ib`, `--ibg`, `--clr-dark`, `--rs`, `--clr-purple`); X button to close; `error?: boolean` prop; `onUpdateCategory?` / `onDeleteCategory?` for in-modal edit/delete (✏️ on hover, 🗑️ in form); `defaultOpen`/`defaultMode`/`defaultEditId`/`onClose` for management mode (no trigger rendered)
