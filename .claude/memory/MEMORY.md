...
- [Recent Fixes](project-recent-fixes.md) — formatCurrency decimals, TransactionList members prop (required!), updateColor syncs member.color, HeroCard colors reverted to white
- [Billing & Income Cycle](project-billing-cycle.md) — 10th-to-10th expense cycle + separate calendar-month+1 income cycle for finance `month` filtering; single entry point `isInFinanceCycle()`; per-user billing day planned but not built
