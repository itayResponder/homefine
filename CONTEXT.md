...
- ✅ Kanban board (tasks) — TasksView replaced by `KanbanBoard` with three columns: לבצע / בתהליך / בוצע. Drag-and-drop via `@dnd-kit/core` (PointerSensor + TouchSensor, distance:8 activation). Any member can drag any task. Moving a task to "בתהליך" sets `startedAt`, moving to "בוצע" sets `lastDoneAt` + rotates if rotation task. Task type extended: `status?: TaskStatus`, `dueDate?: string` (YYYY-MM-DD), `estimatedDays?: number`, `startedAt?: number`. Old tasks without `status` are inferred: one-time with `lastDoneAt` → done, else → todo. KanbanCard shows: title, room icon, assignee (colored dot), due date badge (overdue highlighted red), progress bar (in-progress + estimatedDays: green→yellow→red). AddTaskModal extended with optional due date + estimated days fields. Firebase: `moveTaskStatus` in `homeDb.ts`, `moveStatus` in `useTasks`. Components: `KanbanBoard.tsx/.css`, `KanbanCard.tsx/.css`, `KanbanColumn.tsx` all under `src/components/home/tasks/`. **Pending: `firebase deploy --only database` for tasks/shoppingItems rules.**
