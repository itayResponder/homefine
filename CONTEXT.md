...
- ✅ Online presence in header — SyncBar and OnlineBar standalone bars removed. Online users now shown as avatar circles in AppHeader: colored initials circle (color derived from name hash) with a pulsing green dot. Up to 4 shown, "+N" overflow. Appears in both AppPage and HouseholdPage (not Dashboard). `AppHeader` receives `online?: Record<string, { name: string }>` prop. HouseholdPage now runs `usePresence` hook. CSS: `.ah-online`, `.ah-online-avatar`, `.ah-online-dot` in `AppHeader.css`.
